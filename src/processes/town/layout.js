const distanceTransform = require('vendor.distancetransform')
...
class TownLayout extends kernel.process {
    constructor (...args) {
        super(...args)
    ...
    main () {
        ...
        if (layout.isPlanned()) {
            Log.error(`Room ${this.data.room} already planned`)
            layout.visualize()
            return this.pkill()
        }
        ...
        if (this.data.plan >= plans.length) {
            ...
            Log.error(`Unable to come up with a layout for ${this.room.name}`)
            return this.pkill()
        }
    ...
    */
    nearController () {
        ...
        Log.debug(corePosition)
        Log.debug(flower1Position)
        Log.debug(flower2Position)
    ...
    */
    planLayout (corePos, flower1Pos, flower2Pos) {
        ...
        if (!this.planRoads(layout, corePos, flower1Pos, flower2Pos)) return this.pkill()

        layout.save()
        Log.info(`Room planning for room ${this.data.room} has successfully completed`)
        return this.pkill()
    }
    ...
    */
    planRoads (layout, corePos, flower1Pos, flower2Pos) {
        Log.info(`Planning roads for room: ${this.data.room}`)
    ...
    */
    planRoad (layout, fromPos, toPos, matrix) {
        ...
        if (!path.path || path.path.length < 1) {
            Log.error(`Unable to find path for road from: ${fromPos} to: ${toPos} in room: ${this.data.room} path returned was: ${JSON.stringify(path)}`, 'layout')
            return false
        } else Log.info(`Planning road from: ${fromPos} to: ${toPos} in room: ${this.data.room} with length ${path.path.length}`, 'layout')
