module.exports = class RoomFurnisher {
    ...
    getNextVacancy() {
        const self = this;
        const structs = self.room.find(FIND_MY_STRUCTURES, {
            filter(structure) {
        ...
        // 100 attempts
        for (let i = 0; i < 50; i++) {
            const rand = structs[Math.floor(Math.random() * structs.length)];
            ...
            const pos = self.room.getPositionAt(x, y);
            const occupied = _.filter(pos.lookFor(LOOK_STRUCTURES), structure => structure.structureType !== 'road');
    ...
    runExtensionBuilder() {
        ...
        if (
            ...
            const pos = this.getNextVacancy(self.room);
            if (pos) {
                const code = pos.createConstructionSite(STRUCTURE_EXTENSION);
                if (code === 0) {
    ...
    roadPlanner() {
        ...
        for (const spawn of room.find(FIND_MY_SPAWNS)) {
            ...
            if (room.childRooms) {
                for (const child of room.childRooms) {
                    const childRoom = Game.rooms[child.childRoom];

                    if (childRoom && childRoom.memory.connected === undefined) {
                        const path = PathFinder.search(childRoom.sources[0].pos, spawn.pos).path;
                        path.forEach((tile) => {
                            const pos = new RoomPosition(tile.x, tile.y, tile.roomName);
    ...
    surroundStructures(spawn, room) {
        const self = this;
        let pos; let leftx; let rightx; let topy; let
            boty;

        const structures = self.room.find(FIND_MY_STRUCTURES, {
            filter: s => (
                s.structureType !== STRUCTURE_ROAD
                && s.structureType !== STRUCTURE_WALL
            )
        });
        const terrain = structures.map((structure) => {
            pos = structure.pos;
