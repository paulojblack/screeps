...
class TwoodOSKernel {
    ...
    launchProcess (name, data = {}, parent = false) {
        const pid = this.sched.getNextPid();

        this.mem.processes.index[pid] = {
            n: name,
            d: data,
            p: parent
        };
        ...
        const priority = this.sched.getPriorityForPid(pid)

        if (!this.mem.processes.queues[priority]) {
            this.mem.processes.queues[priority] = []
        }
    ...
    getProcessMemory(process) {
        this.mem.processMemory = this.mem.processMemory || {};
        let pid = p.pid;
        this.mem.processMemory[pid] = this.mem.processMemory[pid] || {};
        return this.mem.processMemory[pid];
    };

    storeProcessTable() {
        let aliveProcess = _.filter(_.values(this.processTable), (p) => p.status!= ProcessStatus.DEAD);
        this.mem["processTable"] = _.map((p) => (p.pid, p.parentPID, p.className));
    };

    loadProcessTable() {
        reboot();
        let storedTable = this.mem["processTable"];
        for (const item of storedTable) {
            let pid, parentPID, className;

            try {
                let processClass = require(`processes.${process}`);
                ...
                Log.error("Error when loading:" + e.message);
                Log.error(className);
            }
        }
    };
