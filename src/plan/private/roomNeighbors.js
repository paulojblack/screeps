...
module.exports = function (room) {
    const [fullRoomName, latDir, latVal, longDir, longVal] = room.name.match(/([A-Z])(\d*)([A-z])(\d*)$/);
    const neighbors = {
        left: {
            accessible: room.find(FIND_EXIT_LEFT).length > 0,
            name: getLeftNeighborRoomName(latDir, latVal, longDir, longVal)
        },
        right: {
            accessible: room.find(FIND_EXIT_RIGHT).length > 0,
            name: getRightNeighborRoomName(latDir, latVal, longDir, longVal)
        },
        top: {
            accessible: room.find(FIND_EXIT_TOP).length > 0,
            name: getTopNeighborRoomName(latDir, latVal, longDir, longVal)
        },
        bottom: {
            accessible: room.find(FIND_EXIT_BOTTOM).length > 0,
            name: getBottomNeighborRoomName(latDir, latVal, longDir, longVal)
        }
    };

    neighbors.left.visible = Game.rooms[neighbors.left.name] !== undefined;
    neighbors.right.visible = Game.rooms[neighbors.right.name] !== undefined;
    neighbors.top.visible = Game.rooms[neighbors.top.name] !== undefined;
    neighbors.bottom.visible = Game.rooms[neighbors.bottom.name] !== undefined;
